package zio.telemetry.opentelemetry.bootstrap
...
/** OtelProviders is a case class that holds the OpenTelemetry providers for
...
trait ZIOpenTelemetryApp extends ZIOApp {
  ...
  /** The OpenTelemetry logging layer for the ZIOpenTelemetry trait.
    ...
    * trait, this layer is provided, and will be used by bootstrap.
    */
  protected def otel4zLogging
      : URLayer[api.OpenTelemetry with ContextStorage, Unit] = ZLayer.unit

  protected def zioMetrics
      : URLayer[api.OpenTelemetry with ContextStorage, Unit] = ZLayer.unit

  /** The OpenTelemetry [[io.opentelemetry.sdk.logs.SdkLoggerProvider]] for the
    * ZIOpenTelemetry trait.
    *
    * By default, no logger provider is provided. You can override this to
    * provide a logger provider, e.g. to export logs in OTLP gRPC format to
    * collector.
    *
    * Or mixing in the [[Logging]] trait, which provides a logger provider that
    * exports logs in OTLP gRPC format to collector.
    */
  ...
  protected def tracerProvider: URIO[Scope, Option[SdkTracerProvider]] =
    ZIO.none
  ...
  /** The OpenTelemetry providers for the ZIOpenTelemetry trait.
    *
    * This is the layer that will be used to provide the OpenTelemetry providers
    * to the ZIO application. It includes the OpenTelemetry logger provider, the
    * OpenTelemetry meter provider, and the OpenTelemetry tracer provider.
    */
  ...
  /** The base log layer.
    *
    * Default implementation does nothing, hence uses the default ZIO console
    * logger, which logs to stdout.
    *
  ...
  /** The OpenTelemetry metrics layer for the ZIOpenTelemetry trait.
    *
    * This is a no-op implementation, which does not export metrics, when mixin
    * the [[Metrics]] trait, it will export metrics in OTLP gRPC
    */
  def otel4zMetrics(
      instrumentationScopeName: String
  ): URLayer[io.opentelemetry.api.OpenTelemetry & ContextStorage, Meter] =
    ZLayer.succeed(noop.NoopMeter(instrumentationScopeName))

  /** The OpenTelemetry tracing layer for the ZIOpenTelemetry trait.
    *
    * This is a no-op implementation, which does not export traces, when mixin
    * the [[zio.telemetry.opentelemetry.tracing.Tracing]] trait, it will export
    * traces in OTLP gRPC.
    */
  def otel4zTracing(
      instrumentationScopeName: String
  ...
  /** Customize the OpenTelemetry builder.
    *
    * This method is called by the [[openTelemetryLive]] layer to customize the
    * OpenTelemetry builder.
    *
    * You can override this method to customize the OpenTelemetry builder, e.g.
    * to add a custom resource, or to add a custom span processor.
    */
  def customizeOpenTelemetry(builder: OpenTelemetrySdkBuilder): Unit = {}

  /** The OpenTelemetry layer for the ZIOpenTelemetry trait.
    *
    * This is the layer that will be used to provide the OpenTelemetry to the
    * ZIO application.
    */
  def openTelemetryLive: ZLayer[OtelProviders, Nothing, OpenTelemetrySdk] =
    ZLayer.scoped[OtelProviders](
      for {
  ...
  def observabilityLayer
      : ULayer[ContextStorage & OpenTelemetrySdk & Meter & Tracing] =
    baseLogLayer >>>
      OpenTelemetry.contextZIO >+> (otelProviders >>>
        openTelemetryLive) >+> (otel4zLogging ++ zioMetrics ++ otel4zMetrics(
        resourceName
